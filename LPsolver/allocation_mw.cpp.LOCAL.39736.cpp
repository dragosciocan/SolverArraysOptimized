    ...
    */
    ...
    long double AllocationMW::CalculateGlobalMWProblemOpt() {
        ...
        for (int a = 0; a < (*solution_).size(); ++a) {
            for (int i = 0; i < (*solution_)[a].size(); ++i) {
    ...
    void AllocationMW::VerifySolution() {
        ...
        for(j = 0; j < nbImpressions; j++) {      // demand must meet supply
            for(i = 0; i < (*transpose_bids_matrix_)[j].size(); i++) {
                weighted_constraint += ((double) (*transpose_bids_matrix_)[j][i].second) *
                                       ((double) weights_[(*transpose_bids_matrix_)[j][i].first]) *
                                       x[j][(*transpose_bids_matrix_)[j][i].first];
            }
        }
        ...
        IloExpr obj_exp(env);
        for(i = 0; i < nbImpressions; i++) {
            for(j = 0; j < (*transpose_bids_matrix_)[i].size(); j++) {
                obj_exp += ((double) (*transpose_bids_matrix_)[i][j].second) * x[i][(*transpose_bids_matrix_)[i][j].first];
            }
        }
        ...
        */
        ...
        if (abs(cplex.getObjValue() - CalculateGlobalMWProblemOpt()) < 0.00001) {
